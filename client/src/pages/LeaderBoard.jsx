    ...
    <ol className="flex flex-col list-decimal">
      {users.map((user, index) => (
        <li key={user._id} className="flex flex-col justify-center items-center border-royalBlueTop">
          <p className={username === user.username ? "heading text-2xl font-bold text-center" : "text-xl font-bold text-center"}>
            {index === 0 ? '1st' : (index === 1 ? '2nd' : (index === 2 ? '3rd' : (index >= 3 && index <= 10 ? `${index + 1}th` : '')))} {user.username}
          </p>
