...
function compareSpreadBet(bet, jsonData) {
  const matchedData = jsonData.find(
    (data) => normalizeMatchup(data.matchup) === bet.matchup
  );

  if (matchedData) {
    const { winner, spread } = bet;
    const matchedTeamData = matchedData.scores.find(
      (score) => score.team === winner
    );

    if (matchedTeamData) {
      const adjustedScore = parseFloat(matchedTeamData.score) + spread;
      const otherTeamData = matchedData.scores.find(
        (score) => score.team !== winner
      );
...
function compareTotalBet(bet, jsonData) {
  ...
  if (matchedData) {
    const combinedScore = matchedData.scores.reduce(
  ...
  try {
    ...
    for (const bet of expiredBets) {
      ...
      user.activeBets = user.activeBets.filter(
        (activeBetId) => activeBetId.toString() !== bet._id.toString()
      );
