    ...
    try {
      ...
      const simplifiedData = response.data[0]?.events.map((event) => {
        ...
        const homeTeamOutcome =
          event.displayGroups[0]?.markets[pointSpreadIndex]?.outcomes[1];
        const homeTeam = {
        ...
        const totalScoreOutcome =
          event.displayGroups[0]?.markets[totalIndex]?.outcomes[0];
    ...
    try {
      ...
      for (let i = 1; i <= 34; i++) {
        ...
        const teamData = await axios.get(
          `https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/${i}`
        );
        const team = teamData.data.team;

        const logoLink =
          Array.isArray(team.logos) && team.logos.length > 0
            ? team.logos.find((logo) => logo.rel && logo.rel.includes("full"))
                ?.href
            : null;

        const totalRecordItem = team.record?.items?.find(
        ...
        try {
          const teamStatsData = await axios.get(
            `https://sports.core.api.espn.com/v2/sports/football/leagues/nfl/seasons/2021/types/2/teams/${i}/statistics?lang=en&region=us`
          );
          ...
          const passingYardsPerGame = categories[1].stats.find(
            (stat) => stat.name === "passingYardsPerGame"
          );
          const rushingYardsPerGame = categories[2].stats.find(
            (stat) => stat.name === "rushingYardsPerGame"
          );
          const totalPointsPerGame = categories[9].stats.find(
            (stat) => stat.name === "totalPointsPerGame"
          );

          // Add these specific statistics to the teamInfo object and round to the nearest tenth
          teamInfo.passingYardsPerGame = passingYardsPerGame
            ? parseFloat(passingYardsPerGame.value).toFixed(1)
            : "N/A";
          teamInfo.rushingYardsPerGame = rushingYardsPerGame
            ? parseFloat(rushingYardsPerGame.value).toFixed(1)
            : "N/A";
          teamInfo.pointsPerGame = totalPointsPerGame
            ? parseFloat(totalPointsPerGame.value).toFixed(1)
            : "N/A";
        } catch (error) {
