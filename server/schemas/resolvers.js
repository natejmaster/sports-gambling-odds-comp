...
const resolvers = {
  ...
  Mutation: {
    ...
    removeBet: async (parent, { betId }, context) => {
      if (context.user) {
        try {
    ...
    addBet: async (
      parent,
      { betType, matchup, spread, winner, total, endTime, betStatus, units },
      context
    ) => {
      try {
        if (context.user) {
          ...
          if (user.units < units) {
            throw new Error(
              "Insufficient Unit Balance for this Bet!! Please Try Again."
            );
          }
          user.units -= units;
          await user.save();
          ...
          const populatedUser = await User.findById(context.user._id).populate(
            "activeBets"
          );
